...
| 拆出新 crate 后 `t!` 找不到表或运行时回退英文；bin 间 `#[path]` 复用源码 | `rust-i18n`, `i18n!`, `set_locale`, `CURRENT_LOCALE`, `locales`, 多 crate, 翻译表, 回退英文, `#[path]`, bin 共享代码 | [rust-i18n 多 crate](rust-i18n-multi-crate.md) |
...
| 升级 Zed rev 后 debug 构建报 `missing: compressed` / `compressed is not a member of trait RustEmbed`，release 正常 | `rust-embed`, `compression`, `compressed`, `RustEmbed`, `fs_embed`, `debug-embed`, `E0046`, `E0407`, `assets`, `Zed 升级` | [Zed 升级后 rust-embed compression 打断 debug 构建](zed-rust-embed-compression-debug.md) |
