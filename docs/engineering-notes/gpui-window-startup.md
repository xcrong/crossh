...
CLI 的另一个生命周期独立于 GPUI：如果终端进程本身进入 `Application::run`，命令自然会被 GUI 事件循环阻塞。要实现普通桌面 CLI 的启动语义，终端进程应只负责生成独立 GUI 子进程，然后立即返回；子进程必须有内部标记以避免递归生成自身。

## 稳定规则

1. 在应用启动回调中创建首个窗口时，直接调用 `cx.open_window`。
2. 只有在已经存在其他窗口、且调用来自当前 UI 事件时，才使用 `cx.defer` 延后创建新窗口。
3. CLI 启动 GUI 时，父进程生成同一可执行文件的子进程后返回。子进程使用内部环境标记进入真实 GUI 路径，并断开标准输入、输出和错误流。
...
- CLI 命令构造测试应验证子进程重新进入正确的子命令、继承目标工作目录，并带有防递归标记。
...
- CLI 进程分离：`src/features/git/cli.rs`
- 启动路径装配：`src/main.rs`
