...
cargo build "${CARGO_ARGS[@]}" --bin crossh --bin crossh-agent --bin crossh-git --bin crossh-updater
...
cp "$BIN_DIR/crossh-agent" "$MACOS/crossh-agent"
...
codesign --force --sign - --identifier "$BUNDLE_ID.agent" "$MACOS/crossh-agent"
