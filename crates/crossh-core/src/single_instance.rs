//! 单实例复用：回环 TCP + 端口文件。
//!
//! 第二个进程把请求发给已在运行的首个实例后退出；
//! 无运行实例时返回 [`ForwardOutcome::NoInstance`]，调用方继续以主实例启动。
//! 主窗口（`instance.port`）、Git Viewer（`git.port`）、Note Viewer（`note.port`）
//! 共用同一机制，只是端口文件不同，经各自的 `try_forward_*` / `serve_*` 接入。
//!
//! 本模块零 `gpui` 依赖。线程到前台的投递桥（channel → `cx.spawn`
//! 前台循环）由调用方组装，`serve` 只接受 `Fn` 回调。
//!
//! 设计取舍（保持无聊、可测）：
...
/// 主窗口端口文件路径：每用户缓存目录下的固定位置。
pub fn port_file_path() -> PathBuf {
    port_dir().join("instance.port")
}

/// Git Viewer 端口文件：与主窗口分文件，避免互相抢占。
pub fn git_port_file_path() -> PathBuf {
    port_dir().join("git.port")
}

/// Note Viewer 端口文件：与主窗口分文件，避免互相抢占。
pub fn note_port_file_path() -> PathBuf {
    port_dir().join("note.port")
}
...
pub fn resolve_open_path(raw: &Path, cwd: &Path) -> Result<PathBuf, String> {
...
pub fn try_forward(request: Option<&Path>) -> ForwardOutcome {
    try_forward_to(&port_file_path(), request)
}

/// 把 Git Viewer 打开请求转发给已在运行的 `crossh-git` 实例。
pub fn try_forward_git(cwd: &Path) -> ForwardOutcome {
    try_forward_to(&git_port_file_path(), Some(cwd))
}

/// 把 Note Viewer 打开请求转发给已在运行的 `crossh-note` 实例。
pub fn try_forward_note() -> ForwardOutcome {
    try_forward_to(&note_port_file_path(), None)
}
...
pub fn serve(on_request: impl Fn(Option<PathBuf>) + Send + 'static) -> std::io::Result<()> {
    serve_to(port_file_path(), on_request)
}

/// 以 Git 主实例身份监听：`None` 请求不该出现，收到时直接忽略。
pub fn serve_git(on_request: impl Fn(PathBuf) + Send + 'static) -> std::io::Result<()> {
    serve_to(git_port_file_path(), move |request| {
        if let Some(directory) = request {
            on_request(directory);
        }
    })
}

/// 以 Note 主实例身份监听：载荷无意义，任何请求都视为聚焦。
pub fn serve_note(on_request: impl Fn() + Send + 'static) -> std::io::Result<()> {
    serve_to(note_port_file_path(), move |_| on_request())
}
...
mod tests {
    ...
    #[test]
    fn viewer_port_files_do_not_clash_with_main_instance() {
        assert_eq!(
            git_port_file_path()
                .file_name()
        ...
        assert_eq!(
            port_file_path().file_name().and_then(|name| name.to_str()),
            Some("instance.port")
        );
    }
}
