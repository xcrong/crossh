...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    /// Set value without requiring a `Window` (used for programmatic sync like note selection).
    /// Updates `display_map` and clears history; next render will re-layout.
    pub fn set_value_simple(&mut self, value: impl Into<SharedString>, cx: &mut Context<Self>) {
        let text: SharedString = value.into();
        let normalized = self.normalize_input(&text);
        let new_rope = Rope::from(normalized.as_ref());
        let old_text = self.text.clone();
        let range = 0..old_text.len();
        self.text = new_rope.clone();
        self.display_map
            .on_text_changed(&self.text, &range, &new_rope, cx);
        self._pending_update = true;
        self.reset_selection();
        self.undo_manager.clear();
        self.reset_lsp_state();
