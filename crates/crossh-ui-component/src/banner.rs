...
mod tests {
    ...
    #[test]
    fn banner_builder_defaults_to_danger_inline() {
        let banner = Banner::new("test");
        assert_eq!(banner.tone, BannerTone::Danger);
        assert_eq!(banner.layout, BannerLayout::Inline);
        assert!(!banner.compact);
        assert!(banner.title.is_none());
        assert!(banner.actions.is_empty());
    }

    #[test]
    fn banner_builder_sets_all_fields() {
        let banner = Banner::new("test")
        ...
        assert_eq!(banner.layout, BannerLayout::Stacked);
        assert_eq!(banner.title.as_deref(), Some("title"));
        assert_eq!(banner.description.as_deref(), Some("desc"));
        assert!(banner.compact);
    ...
    #[test]
    fn stacked_and_inline_layouts_are_distinct() {
        assert_ne!(BannerLayout::Inline, BannerLayout::Stacked);
    }
}
